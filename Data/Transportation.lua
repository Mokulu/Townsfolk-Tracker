TF_TRANSPORTATION_SPOTS = {
    { zone = ZONE.TIRISFAL_GLADES, x = 0.6070, y = 0.5875, name = "Skull Rock, Durotar", faction = FACTION.HORDE },
    { zone = ZONE.TIRISFAL_GLADES, x = 0.6184, y = 0.5910, name = "Grom'gol Base Camp, Stranglethorn Vale", faction = FACTION.HORDE },
    { zone = ZONE.DUROTAR, x = 0.5087, y = 0.1371, name = "Brill, Tirisfal Glades", faction = FACTION.HORDE },
    { zone = ZONE.DUROTAR, x = 0.5063, y = 0.1273, name = "Grom'gol Base Camp, Stranglethorn Vale", faction = FACTION.HORDE },
    { zone = ZONE.THE_BARRENS, x = 0.6366, y = 0.3860, name = "Booty Bay, Stranglethorn Vale" },
    { zone = ZONE.IRONFORGE, x = 0.7684, y = 0.5124, name = "Deeprun Tram, Stormwind City", faction = FACTION.ALLIANCE },
    { zone = ZONE.WETLANDS, x = 0.0466, y = 0.5718, name = "Auberdine, Darkshore", faction = FACTION.ALLIANCE },
    { zone = ZONE.WETLANDS, x = 0.0510, y = 0.6339, name = "Thereamore Isle, Dustwallow Marsh", faction = FACTION.ALLIANCE },
    { zone = ZONE.DARKSHORE, x = 0.3242, y = 0.4378, name = "Menethil Harbor, Wetlands", faction = FACTION.ALLIANCE },
    { zone = ZONE.DARKSHORE, x = 0.3318, y = 0.4014, name = "Rut'theran Village, Teldrassil", faction = FACTION.ALLIANCE },
    { zone = ZONE.TELDRASSIL, x = 0.5487, y = 0.9670, name = "Auberdine, Darkshore", faction = FACTION.ALLIANCE },
}
